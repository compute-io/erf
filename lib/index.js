...
function evaluate( x, opts ) {
	...
	if ( !isArray( x ) ) {
		throw new TypeError( 'erf()::invalid input argument. Must provide either a single numeric value or a numeric array. Value: `' + x + '`.' );
	}
	...
	if ( clbk ) {
		for ( i = 0; i < len; i++ ) {
			v = clbk( x[ i ], i );
			if ( typeof v !== 'number' ) {
				throw new TypeError( 'erf()::invalid value. Accessed array values must be numeric. Value: `' + v + '`.' );
			}
			arr[ i ] = erf( v );
		}
	} else {
		for ( i = 0; i < len; i++ ) {
			v = x[ i ];
			if ( typeof v !== 'number' ) {
				throw new TypeError( 'erf()::invalid input argument. Input array must only contain numeric values. Value: `' + v + '`.' );
			}
			arr[ i ] = erf( v );
		}
	}
